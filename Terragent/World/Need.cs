...
/// <param name="Stations">The stations it is worked at, and the tile each puts down.</param>
/// <param name="Needs">
...
internal sealed record Need(
    int ItemID,
    int? Yields,
    IReadOnlyList<(Need Need, int TileID)> Stations,
    IReadOnlyList<(Need Need, int Count, IReadOnlyList<Need> Instead)> Needs)
{
