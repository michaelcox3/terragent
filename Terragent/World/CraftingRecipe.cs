...
/// <param name="Stations">
/// Each station: the item that places it, anything else that places the same tile, and the
/// tile itself.
/// </param>
// The book, and a different type from RecipeNode on purpose. A row names its parts and stops;
// a RecipeNode holds them. Sharing one type meant a furnace in a book row reported that nothing
...
internal readonly record struct CraftingRecipe(
    ...
    IReadOnlyList<(int ItemID, IReadOnlyList<int> Instead, int TileID)> Stations);
